...
def jac_of_constraint(x, *args):
    ...
    if args[0] == 'model':
        ...
        if args[1][0] == 'x1':
            ...
            jac_cons[3, i] =  v1[i]*np.sin(theta1[i])*p.dt
            jac_cons[3, i + 1] = 0
            ...
            jac_cons[3, i] =  - v1[i]*np.cos(theta1[i])*p.dt
            jac_cons[3, i + 1] = 0
            ...
            jac_cons[2, i] = -p.d2/2*np.cos(theta[i])*(theta[i+1] - theta[i]) - p.d2/2*np.sin(theta[i])*(-1)
            jac_cons[2, i + 1] = -p.d2/2*np.sin(theta[i])
            jac_cons[4, i] =  v2[i]*np.sin(theta2[i])*p.dt
            jac_cons[4, i + 1] = 0
            ...
            jac_cons[2, i] = -p.d2/2*np.sin(theta[i])*(theta[i+1] - theta[i]) + p.d2/2*np.cos(theta[i])*(-1)
            jac_cons[2, i + 1] = p.d2/2*np.cos(theta[i])
            jac_cons[4, i] =  - v2[i]*np.cos(theta2[i])*p.dt
            jac_cons[4, i + 1] = 0
...
def constraint(x, *args):
    ...
    if args[0] == 'model':
        i = args[1][1]
        if args[1][0] == 'x1':
            value = x[i+1] - x[i]  + p.d2/2*np.sin(theta[i])*(theta[i+1] - theta[i]) - v1[i]*np.cos(theta1[i])*p.dt
        
        elif args[1][0] == 'y1':
            value = y[i+1] - y[i]  - p.d2/2*np.cos(theta[i])*(theta[i+1] - theta[i]) - v1[i]*np.sin(theta1[i])*p.dt
            
        elif args[1][0] == 'theta1':
            value = theta1[i+1] - theta1[i] - omega1[i]*p.dt
            
        elif args[1][0] == 'x2':
            value = x[i+1] - x[i] - p.d2/2*np.sin(theta[i])*(theta[i+1] - theta[i])  - v2[i]*np.cos(theta2[i])*p.dt
        
        elif args[1][0] == 'y2':
            value = y[i+1] - y[i] + p.d2/2*np.cos(theta[i])*(theta[i+1] - theta[i])  - v2[i]*np.sin(theta2[i])*p.dt
