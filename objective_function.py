...
def objective_function(x, *args):
    ...
    sum = 0
    for i in range(p.N):
        sum += 10*(theta[i]/p.theta_max - theta1[i]/p.theta1_max)**2 + (theta[i]/p.theta_max - theta2[i]/p.theta2_max)**2 + (omega1[i] ** 2 / p.omega1_max ** 2) + (omega2[i] ** 2 / p.omega2_max ** 2) + (v1[i] ** 2 / p.v1_max ** 2) + (v2[i] ** 2 / p.v2_max ** 2)
...
def jac_of_objective_function(x, *args):
    ...
    for i in range(p.N):
        jac_f[2, i] = 10*2*(theta[i]/p.theta_max - theta1[i]/p.theta1_max)/p.theta_max + 2*(theta[i]/p.theta_max - theta2[i]/p.theta2_max)/p.theta_max 
        jac_f[3, i] = -10*2*(theta[i]/p.theta_max - theta1[i]/p.theta1_max)/p.theta1_max
        jac_f[4, i] = -2*(theta[i]/p.theta_max - theta2[i]/p.theta1_max)/p.theta2_max
